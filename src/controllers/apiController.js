...
const submitForm = (req, res, next) => {
  ...
  if (Object.keys(requestData).length) {
    ...
    if (
      Object.keys(form.choices).length !==
      Object.keys(requestData.choices).length
    ) {
      console.log("Form choices do not equal request choices");
      const error = new Error(
        "Error in apiController.js/submitForm. Request choices do not line up with form choices."
      );
...
const adminRetrieveForms = (req, res) => {
...
const adminRetrieveUsers = (req, res) => {
...
const adminSetProxyOfUser = (req, res) => {
...
const adminCreateForm = (req, res) => {
  const title = req.body.title;
  const choices = req.body.choices;
  try {
    createForm(title, choices);
    res.sendStatus(200);
  } catch (err) {
    res.sendStatus(400);
  }
